...
export default function Messages({ user }) {
  ...
  return (
    <div className="w-full max-w-[935px] border border-ig-border rounded-lg h-[600px] flex bg-ig-card text-ig-text overflow-hidden">
      {/* Inbox List */}
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-ig-border/50">
          {profiles.map(p => (
            <div 
              ...
              className={`p-4 flex items-center space-x-4 cursor-pointer hover:bg-zinc-100 transition-colors ${activeProfile?.id === p.id ? 'bg-zinc-100' : ''}`}
            >
              <div className="w-10 h-10 rounded-full bg-zinc-100 flex items-center justify-center font-bold text-sm">
          ...
          <>
            ...
            <div className="flex-1 p-4 overflow-y-auto space-y-3">
              {chats.map((c, i) => (
                <div key={i} className={`flex ${c.sender === 'me' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`p-3 rounded-2xl text-sm max-w-xs ${c.sender === 'me' ? 'bg-ig-blue text-white' : 'bg-zinc-100 text-ig-text'}`}>
                    {c.text}
                  </div>
                </div>
              ))}
            ...
            <form onSubmit={sendMessage} className="p-4 border-t border-ig-border flex space-x-2">
              <input
                ...
                className="flex-1 bg-zinc-100 border border-ig-border rounded-full py-2 px-4 text-sm focus:outline-none focus:border-zinc-700"
              />
