...
export default function Profile({ user }) {
  ...
  return (
    <div className="w-full max-w-[935px] space-y-12 text-ig-text">
      {/* Profile Header */}
      <div className="flex md:flex-row flex-col items-center md:space-x-20 space-y-6 md:space-y-0 border-b border-ig-border pb-12">
        <div className="w-32 h-32 rounded-full bg-zinc-100 flex items-center justify-center font-serif font-bold text-4xl text-ig-text">
          {username[0].toUpperCase()}
        </div>
        <div className="space-y-4 text-left">
          <h2 className="text-2xl font-light">@{username}</h2>
          <div className="flex space-x-8 text-sm">
            <span><strong className="text-ig-text">{profilePosts.length}</strong> kiriman</span>
            <span><strong className="text-ig-text">{profilePosts.reduce((acc, p) => acc + (p.likes ? p.likes.length : 0), 0)}</strong> suka diterima</span>
          </div>
          <div>
      ...
      <div className="space-y-4">
        <div className="flex justify-center border-t border-zinc-300 -mt-12 py-3 text-xs tracking-wider uppercase font-semibold text-ig-text space-x-1.5 items-center">
        ...
        {profilePosts.length === 0 ? (
          ...
          <div className="grid grid-cols-3 gap-1 md:gap-7">
            {profilePosts.map(post => (
              <div key={post.id} className="relative aspect-square bg-zinc-100 group overflow-hidden rounded-md cursor-pointer border border-ig-border">
